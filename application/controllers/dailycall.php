...
// Build Header array for dailycall existing customer - start
            $daily_hdr = array('dch_header_id' => $daily_hdr_id,
                'dch_customerid' => $customer_id,
                'dch_leadid' => $leadid,
        ...
        $daily_hdr_insert_status = $this->dailycall_model->save_dailyactivityhdr($daily_hdr);
// Build Header array for dailycall existing customer - END
// Loop start for insert / update the contact person grid  - START 
...
//end visitdetail 	
        $businesscat = array("0" => "BULK", "1" => "INTACT", "2" => "REPACK", "3" => "PART TANKER", "4" => "SINGLE - TANKER", "5" => "SMALL PACKING");
        $businesscatprod = array("0" => "bulk", "1" => "intact", "2" => "repack", "3" => "part_tanker", "4" => "single_tanker", "5" => "small_packing");
        $poten_catprod = array("0" => "173794", "1" => "173795", "2" => "1", "3" => "3", "4" => "681045", "5" => "681046");
                ...
                if ($_POST['visitdate'] != "") {
                    if ($hdn_grid_row_data[$key]['dct_new_prod'] == '1') {
                        for ($i = 0; $i <= 5; $i++) {
                            ...
                            if (($hdn_grid_row_data[$key][$businesscatprod[$i]] == "undefined") || ($hdn_grid_row_data[$key][$businesscatprod[$i]] == "")) {
                                $hdn_grid_row_data[$key][$businesscatprod[$i]] = 0;
                            }
                            $insert_dailycall = 1;
                            ...
                            $dcprodinsert[$i]['dct_customer_potential'] = $hdn_grid_row_data[$key][$businesscatprod[$i]];
                        ...
                        $lpid_seq = $this->Leads_model->GetNextlpid($insert_lead_id);
                        /* $businesscatprod=array("0"=>"bulk", "1"=>"intact","2"=>"repack","3"=>"part_tanker","4"=>"single_tanker","5"=>"small_packing");
                          $poten_catprod = array("0"=>"173794", "1"=>"173795","2"=>"1","3"=>"3","4"=>"681045","5"=>"681046"); */
                        $itemgroup_name = $this->Leads_model->GetItemgroup($hdn_grid_row_data[$key]['dct_prodid']);
                        for ($ld_pot = 0; $ld_pot <= 5; $ld_pot++) { /* Insert 5 more rows into lead_prod_potential_types start */
                            ...
                            $slead_prod_poten_type[$ld_pot]['product_type_id'] = $poten_catprod[$ld_pot];

                            if ($hdn_grid_row_data[$key][$businesscatprod[$ld_pot]] != "") {
                                $slead_prod_poten_type[$ld_pot]['potential'] = (float) $hdn_grid_row_data[$key][$businesscatprod[$ld_pot]];
                            } else {
                            ...
                            $lead_customer_pontential[$ld_pot]['id'] = $insert_lead_id;
                            $lead_customer_pontential[$ld_pot]['line_id'] = $lpid_seq;
                            $lead_customer_pontential[$ld_pot]['user1'] = strtoupper($duser);
                            $lead_customer_pontential[$ld_pot]['customergroup'] = $hdncustomergroup;
                            $lead_customer_pontential[$ld_pot]['itemgroup'] = $itemgroup_name['itemgroup'];

                            $lead_customer_pontential[$ld_pot]['customer_number'] = $customer_number;
                            $lead_customer_pontential[$ld_pot]['customer_name'] = $customer_name;

                            $lead_customer_pontential[$ld_pot]['types'] = "LEAD";
                            $lead_customer_pontential[$ld_pot]['collector'] = strtoupper($branch);
                            $lead_customer_pontential[$ld_pot]['lead_created_date'] = date('Y-m-d:H:i:s');
                            $lead_customer_pontential[$ld_pot]['user_code'] = $login_user_id;
                            $lead_customer_pontential[$ld_pot]['businesscategory'] = $businesscat[$ld_pot];
                            if ($hdn_grid_row_data[$key][$businesscatprod[$ld_pot]] != "") {

                                $lead_customer_pontential[$ld_pot]['yearly_potential_qty'] = ($hdn_grid_row_data[$key][$businesscatprod[$ld_pot]] * 12);
                            } else {

                                $lead_customer_pontential[$ld_pot]['yearly_potential_qty'] = 0;
                            }
                        }
                        /* Insert 5 more rows into lead_prod_potential_types End */
                        $ld_pot = 0;
                        $k = 0;
                        $lead_pord_poten_id = $this->dailycall_model->save_dcleadprodpotentypes($slead_prod_poten_type);
                        ...
                        for ($j = 0; $j <= 5; $j++) {
                            ...
                            $dcprodupdate[$j]['dct_customer_potential'] = (float) $hdn_grid_row_data[$key][$businesscatprod[$j]];
...
// start for Payment Collection insert -START 	


            foreach ($hdn_grid_row_data_pay as $key => $val) {
                if ($hdn_grid_row_data_pay[$key]['pay_discussion_points'] != "null") {
                    $pay_collection[$key]['dcp_pay_cust_trx_id'] = $hdn_grid_row_data_pay[$key]['dcp_pay_id'];
                    $pay_collection[$key]['dcp_pay_cust_id'] = $customer_id;
                    $pay_collection[$key]['dcp_pay_customergroup'] = strtoupper($hdncustomergroup);
                    $pay_collection[$key]['dcp_pay_invoice_no'] = $hdn_grid_row_data_pay[$key]['invoice_no'];
                    $pay_collection[$key]['dcp_pay_invoice_date'] = $hdn_grid_row_data_pay[$key]['invoice_date'];
                    $pay_collection[$key]['dcp_pay_due_date'] = $hdn_grid_row_data_pay[$key]['pay_due_date'];
                    $pay_collection[$key]['dcp_pay_discussion_points'] = $hdn_grid_row_data_pay[$key]['pay_discussion_points'];
                    $pay_collection[$key]['dcp_pay_created_date'] = date('Y-m-d:H:i:s');
                    $pay_collection[$key]['dcp_pay_created_userid'] = $login_user_id;
                    $pay_collection[$key]['dcp_pay_created_username'] = $creationuser;
                }
            }
            //	echo"<pre> insert pay_collection";print_r($pay_collection);echo"</pre>";
            //$pay_collecid = $this->dailycall_model->update_payment_collection($pay_collection,$customer_id);
            //	$del = $this->dailycall_model->delete_payment_collection(@$hdncustomergroup);
            if (isset($pay_collection)) {
                $pay_collecid = $this->dailycall_model->save_payment_collection($pay_collection);
            }
// start for Payment Collection insert -END
            ...
            foreach ($hdn_grid_row_data as $key => $val) {
                ...
                $business_cat_type = array("0" => "BULK", "1" => "INTACT", "2" => "REPACK", "3" => "PART TANKER", "4" => "SINGLE - TANKER", "5" => "SMALL PACKING");
                $dc_logdtl_id = $this->dailycall_model->GetNextLogDtlSeqVal('dailycall_log_dtl');
                ...
                for ($ld_pot = 0; $ld_pot <= 5; $ld_pot++) { // Insert 5 more rows into lead_prod_potential_types start
                    ...
                    $daily_logdtl[$ld_pot]['dtllog_prodid'] = $hdn_grid_row_data[$key]['dct_prodid'];
                    //	$daily_logdtl[$ld_pot]['dtllog_prodid'] = $prodinsertdata['productid'];
                    ...
                    $daily_logdtl[$ld_pot]['dtllog_poten'] = $hdn_grid_row_data[$key][$businesscatprod[$ld_pot]];
    ...
    function update_lead_dailycall() {
        ...
        $modofcont_id = $_POST['modeofcontact'];
        $moc_name = $_POST['hdn_mocname'];
        $assignedto = $_POST['hdn_assign_to'];

        $visitdetails = $this->dailycall_model->get_visittypenames($typeofvisit);

        $assign_to_array = $this->Leads_model->GetAssigntoName($assignedto);
        ...
        $cust_account_id = $this->Leads_model->CheckNewCustomer($customerid);
...
//								echo"check".$val['contactperson'];
            ...
            $daily_preson[$key]['dc_leadid'] = $leadid;
        ...
        foreach ($hdn_grid_row_data as $key => $val) {
            ...
            if ($hdn_grid_row_data[$key]['dct_detail_id'] == 'undefined' || $hdn_grid_row_data[$key]['dct_detail_id'] == "") {
                ...
                for ($ld_pot = 0; $ld_pot <= 5; $ld_pot++) { // Insert 5 more rows into lead_prod_potential_types start
                    ...
                    //  echo"checking ".(float)$hdn_grid_row_data[$key][$businesscat[$ld_pot]]."<br>";
                    if ((float) $hdn_grid_row_data[$key][$businesscat[$ld_pot]] != "") {
                    ...
                    $lead_customer_pontential[$ld_pot]['id'] = $insert_lead_id;
                    $lead_customer_pontential[$ld_pot]['line_id'] = $prdetid;
                    $lead_customer_pontential[$ld_pot]['user1'] = strtoupper($duser);
                    $lead_customer_pontential[$ld_pot]['customergroup'] = $hdncustomergroup;
                    $lead_customer_pontential[$ld_pot]['itemgroup'] = $itemgroup_name;
                    $lead_customer_pontential[$ld_pot]['customer_number'] = $customer_number;
                    $lead_customer_pontential[$ld_pot]['customer_name'] = $customer_name;
                    $lead_customer_pontential[$ld_pot]['types'] = "LEAD";
                    $lead_customer_pontential[$ld_pot]['collector'] = $branch;
                    $lead_customer_pontential[$ld_pot]['lead_created_date'] = date('Y-m-d:H:i:s');
                    $lead_customer_pontential[$ld_pot]['user_code'] = $login_user_id;
                    $lead_customer_pontential[$ld_pot]['businesscategory'] = $business_cat_type[$ld_pot];

                    if ($hdn_grid_row_data[$key][$businesscat[$ld_pot]] != "") {

                        $lead_customer_pontential[$ld_pot]['yearly_potential_qty'] = ($hdn_grid_row_data[$key][$businesscat[$ld_pot]] * 12);
                    } else {
                        //echo"in else --- ";
                        $lead_customer_pontential[$ld_pot]['yearly_potential_qty'] = 0;
                    }
                ...
                for ($i = 0; $i <= 5; $i++) {
                    ...
                    $lead_customer_pontential[$i]['customer_number'] = $customer_number;
                    $lead_customer_pontential[$i]['customer_name'] = $customer_name;
    ...
    function dcupdateexcustomer($customergroup, $leadid = 0) {
        ...
        if (!$this->admin_auth->logged_in()) {
            ...
            $activitydata['cust_account_id'] = $this->dailycall_model->get_cust_account_id($customer_id);
            $cust_account_id = $activitydata['cust_account_id'];
            ...
            //$activitydata['cust_payment_data']= $this->dailycall_model->get_payement_form_data_bycstid($cust_account_id);
            $activitydata['cust_payment_data'] = $this->dailycall_model->get_payement_form_data_bygrp($customer_group);
    ...
    function updatedailycall() {
        ...
        //$leadid = $_POST['hdn_leadid'];
        $customer_id = $_POST['hdn_customerid'];
        echo $customergroup = $this->dailycall_model->get_customergroup($customer_id);
        /* $leadid = $_POST['hdn_leadid'];
          $hdr_comments=$_POST['hdn_cmnts'];
          $login_user_id = $this->session->userdata['user_id'];
          $login_username = $this->session->userdata['username'];
        ...
        //redirect('dailycall/viewdiallycalldetailsbydate/'.$customer_id);
        redirect('dailycall/customerdetails/' . $customergroup . '/' . $leadid);
    }
    ...
    function customerdetails($customergroup, $leadid = 0) {
        ...
        if (!$this->admin_auth->logged_in()) {
            ...
            //$activitydata['customerinfo'][0]['credit_limit']=number_format($this->dailycall_model->get_customer_creditlimit($customer_id));
            //	$activitydata['customerinfo'][0]['credit_limit']=1090;
    ...
    function checkdate($date, $customergroup, $username) {
        ...
        $activitydata['response'] = $this->dailycall_model->check_duplicates_daillycall($visit_date, $customername, $login_username);
...
//custname	VIKRAM RESINS & POLYMERS customergroup	VIKRAM RESINS & POLYMERS customerid	1303
    ...
    function edit($customer_id) {
        ...
        if (!$this->admin_auth->logged_in()) {
            ...
            $activitydata['exe_name'] = $user_name;
            $activitydata['customerinfo'] = $this->dailycall_model->get_customerdetails_edit($customer_id, $leadid);
    ...
    function get_dataitemmaster() {
        ...
        $sql = 'SELECT  DISTINCT on (description) id::character varying, description FROM view_tempitemmaster ORDER BY description asc';
    ...
    function get_datacustomer($customergroup) {
        ...
        $sql = "SELECT id,customer_name FROM customermasterhdr WHERE trim(REPLACE(customergroup,'''','')) ='" . trim($customergroup) . "'";
        //	echo $sql;
