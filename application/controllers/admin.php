...
//          echo"not a admin user"; die;
           ...
           // echo"<pre>";echo json_encode($this->data);echo"</pre>"; 
          //  $user_data =json_encode($this->data['users']);
            
            $user_data['data'] = "{\"data\":" . json_encode($this->data['users']) . "}";
           // print_r($user_data); die;
            //$this->_render_page('admin/userdetails', $user_data);
            $this->load->view('admin/userdetails', $user_data);
            //redirect('coming_soon', 'refresh');
        }
    }

    function updateuser_designation($id,$designation)
    {

        $id=$id;
        $designation=urldecode($designation);
        //$this->admin_auth->update_user_designation($id,$designation);
        $db2 = $this->load->database('forms', TRUE);
        $sql = "update dusermaster set designation='" . $designation . "' where header_user_id =" . $id;
        $result = $db2->query($sql);
        if ($this->db->trans_status()) {
            $this->admin_auth->set_message('User Updated Sucessfully');
            $this->session->set_flashdata('message', 'User designation Updated Sucessfully');
            $message ='User designation Updated Sucessfully';
